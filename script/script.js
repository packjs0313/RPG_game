...
// 스탯 포인트 경고 메시지 표시 함수
function showStatPointWarning(msg) {
    const el = document.getElementById('stat-point-warning');
    el.textContent = msg;
    el.classList.remove('shake');
    // 트리거를 위해 강제로 reflow
    void el.offsetWidth;
    el.classList.add('shake');
    clearTimeout(showStatPointWarning._timer);
    showStatPointWarning._timer = setTimeout(() => {
        el.textContent = "";
        el.classList.remove('shake');
    }, 1500);
}
